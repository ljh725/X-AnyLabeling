...
class DigitShortcutDialog(QtWidgets.QDialog):
    """Dialog for managing digit shortcuts with pagination support."""
    ...
    def __init__(self, parent=None):
        ...
        Args:
            parent: The parent widget (LabelingWidget).
        """
        ...
        if (
            ...
            for key, value in self.parent.drawing_digit_shortcuts.items():
                try:
                    normalized_key = int(key)
                except (TypeError, ValueError):
                    continue
                self.digit_shortcuts[normalized_key] = value

        # Sync page state from parent's page manager
        ...
        header_label = QtWidgets.QLabel(
            self.tr(
                "Configure digit shortcuts for quick shape creation (paged):"
            )
        )
    ...
    def _init_page_state(self):
        ...
        """
        # Try to get page state from parent's page manager
        if hasattr(self.parent, "digit_page_manager"):
            page_manager = self.parent.digit_page_manager
            self.pages = page_manager.total_pages
        ...
        if self.digit_shortcuts:
            try:
                max_index = max(self.digit_shortcuts.keys())
                required_pages = max_index // self.page_size + 1
                self.pages = max(self.pages, required_pages)
            except (TypeError, ValueError):
                pass
    ...
    def save_current_page_data(self):
        """Store the current page configuration in memory."""
        start_index = self.current_page * self.page_size
        for row in range(self.page_size):
    ...
    def load_page(self, page_index):
        """Populate the table with values from the selected page."""
        if self.pages <= 0:
            self.pages = 1

        page_index = max(0, min(page_index, self.pages - 1))
        ...
        for row in range(self.page_size):
            ...
            label_edit.setEnabled(mode_combo.currentData() is not None)
            label_edit.setStyleSheet("")

            mode_combo.blockSignals(False)
            label_edit.blockSignals(False)

            self.on_mode_changed(row, mode_combo.currentIndex())
    ...
    def save_settings(self):
        ...
        if error_info:
            page_index, row = error_info
            actual_index = page_index * self.page_size + row
        ...
        if original_page != self.current_page:
            self.load_page(original_page)

        result = {
            index: data
            for index, data in self.digit_shortcuts.items()
            if data.get("mode") is not None and data.get("label")
        }
        self.digit_shortcuts = result
        ...
        if hasattr(self.parent, "digit_page_manager"):
            ...
            # Update total pages in config
            self.parent._config["digit_shortcut_pages"] = self.pages
