...
enabling users to configure more shortcuts while maintaining backward compatibility.

When only one page is configured, behavior is identical to the original single-page system.

Classes:
    - DigitShortcutPageManager: Core logic for page management and index calculation
"""
...
class DigitShortcutPageManager(QtCore.QObject):
    ...
    Signals:
        page_changed: Emitted when the current page changes, with (new_page, total_pages)
    ...
    def __init__(
        ...
        Args:
            ...
            shortcuts: Dictionary of digit shortcuts (index -> {mode, label})
            status_callback: Callable for displaying status messages (message, duration_ms)
        ...
        logger.debug(
            f"DigitShortcutPageManager initialized: "
            f"pages={self._total_pages}, shortcuts={len(self._shortcuts)}"
        )
    ...
    def _calculate_total_pages(self) -> int:
        ...
        config_pages = self._config.get("digit_shortcut_pages", self.DEFAULT_PAGES)
        try:
        ...
        if self._shortcuts:
            try:
                ...
                required_pages = max(required_pages, (max_index // self.PAGE_SIZE) + 1)
            except (TypeError, ValueError):
                pass
    ...
    def get_actual_index(self, digit_num: int) -> int:
        ...
        if not 0 <= digit_num <= 9:
            logger.warning(f"Invalid digit_num: {digit_num}, expected 0-9")
            return digit_num
    ...
    def switch_page(self) -> Tuple[int, int]:
        ...
        logger.debug(
            f"Page switched: {old_page + 1} -> {self._current_page + 1} "
            f"(total: {self._total_pages})"
        )
    ...
    def get_status_message(self, switch_key: str = "F1") -> str:
        ...
        status_message = self._tr("Digit shortcuts: Page {page}/{total} ({mapping})").format(
            page=info["display_page"],
            total=info["total_pages"],
            mapping=mapping_text,
        )
        
        hint_message = self._tr("Use {key} to switch pages").format(key=switch_key)
