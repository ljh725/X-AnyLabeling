...
Deployed into the host project from ``migration/``.  The function
:func:`layout_category` is wired into :func:`apply_layout` in
``pose_layout.py`` under the ``"category"`` branch; the settings panel
exposes it as the ``"类别分组"`` button.

Design contract
----------------
``layout_category`` keeps its constants (*coco_keypoint_index*,
*body_parts*) as explicit parameters rather than importing them, so the
module stays decoupled from how the host names/organises those
constants — the dispatcher simply passes the project's own
``COCO_KEYPOINT_INDEX`` / ``BODY_PARTS`` (defined in ``pose_constants``)
at call time.

The host must provide a ``PoseLabelItem`` dataclass with at least these
attributes::
