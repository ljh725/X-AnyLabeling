...
1. 按 `Ctrl+Shift+E` 开启“矩形边编辑”。如果当前处于绘制模式，系统会先切回编辑模式。
2. 将鼠标移动到任意可见矩形的边附近。可命中的边显示白色细线，鼠标变为指向手形。
3. 按住左键并移动至少 4 个屏幕像素。越过阈值后，鼠标变为闭合手形并开始拖动该边。
4. 松开左键提交修改。一次完整拖拽对应一次撤销记录。

边预选和拖动不要求先选中矩形，也不会因为当前选中了其他对象而失效。按下一条边时，系统会清除正式对象选择，使“拖边”和“拖对象”不会在同一手势中同时发生。
...
- 仅按下、尚未越过 4 px 阈值时按 `Esc`：取消 pending 状态，不改变几何；
...
stateDiagram-v2
    ...
    PENDING --> HOVER: 松开且未越过 4px
    PENDING --> DRAGGING: 屏幕位移达到 4px
...
1. 边距离，距离更近者优先；
2. 矩形面积，面积更小者优先；
3. 画布堆叠顺序，更新且更靠上的对象优先。

按下时优先重新验证当前白色预选边，避免相邻窄框之间因轻微鼠标抖动切换目标。

### 4.3 顶点优先

任何可交互形状的顶点都全局优先于矩形边。鼠标位于矩形角点、其他矩形角点或多边形顶点附近时，不启动边编辑，以保留原有顶点编辑语义。
...
4 px 拖动启动阈值和 1 px 最小矩形尺寸当前是代码常量，不是用户配置项。
...
| `tests/test_rect_edge_canvas_semantics.py` | 32 | 未选中预选、pending 阈值、相邻边锁定、顶点优先、图像边界、失焦回滚、模式切换 |
...
直接拖边提供可视预选、pending 阈值、取消回滚、精修降速及独立状态机，是主要的交互式单边编辑方式。

## 14. 维护约束

1. `RectEdgeRef` 和交互状态不得序列化。
2. 几何修改必须走规范化与反翻转逻辑，并使 `Shape` 缓存失效。
3. 鼠标命中必须保留全局顶点优先和不可交互对象过滤。
4. pending 使用屏幕像素阈值，hit-test 使用随缩放换算的图像阈值。
