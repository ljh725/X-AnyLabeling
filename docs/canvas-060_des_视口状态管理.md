...
2. **重置从当前到末尾图像视图**
   ...
   - 为非当前图片增加一次性“下次强制默认视图”标记，避免被上一张图片继承
   - 如果当前图片在范围内，立即恢复默认视图
...
- 非当前图片还会在下一次加载时跳过 `keep_prev_viewport`、`keep_prev_scale` 和旧滚动值继承；该标记消费后恢复正常视口保存

### 7.4 与 Navigator Widget 的关系
- 视口变化时更新导航器视口矩形
- 代码位置: `label_widget.py` 第 5180-5206 行

### 7.5 数据集生命周期
- 切换文件夹、打开独立图片或关闭数据集时，会清理旧数据集的视口历史和待默认打开标记
- 重新打开旧文件夹不会自动复活已清理的旧视角
...
同时会选中 `Fit Window`、取消 `Fit Width`，保证菜单状态与画布一致。

因此，用户执行“重置当前图像视图”后，通常会看到画面缩放回适应窗口，并回到默认起始位置。

### 10.2 状态反馈

重置操作不会弹出确认框，而是通过状态栏显示本地化提示，例如：

1. `Reset 1 image view(s) in the current image`
2. `Reset 12 image view(s) in the current image to the end`
3. `Reset 84 image view(s) in all images`

这里的数量是有效目标图片数量；即使图片原先没有缓存，只要当前画布被恢复，也会计为 1 张。
...
状态只存在于当前数据集的内存会话中，使用规范化文件身份关联：

| 状态 | 含义 | 下次加载行为 |
|------|------|--------------|
| `UNKNOWN` | 没有该图片的精确历史，也没有待重置意图 | 按继承策略或默认视图处理 |
| `CACHED` | 有该图片自己的缩放模式、缩放值和图像坐标中心 | 优先恢复自己的精确历史 |
| `RESET_PENDING` | 用户明确要求该图片下次默认打开 | 禁止继承旧视口，成功默认加载后消费标记 |

同一个文件不能同时处于 `CACHED` 和 `RESET_PENDING`。
...
EXACT（当前图片精确历史）
    ↓
PREVIOUS_VIEWPORT（A：继承上一张完整视口）
    ↓
PREVIOUS_SCALE（只继承上一张缩放）
    ↓
DEFAULT（适应窗口、默认起始位置）
```

A 只控制 `PREVIOUS_VIEWPORT` 是否可用，不控制精确历史的保存和恢复。B 是最高优先级的失效事务，不是简单删除字典记录。

### 12.3 状态转换

```text
UNKNOWN ──离开图片并成功捕获──► CACHED
CACHED ──执行 B 重置──────────► RESET_PENDING
UNKNOWN ──执行 B 重置──────────► RESET_PENDING
RESET_PENDING ──默认加载成功──► UNKNOWN
UNKNOWN/CACHED ──数据集边界───► 清空
```

默认视图成功应用后，图片再次被用户调整并离开时，会重新进入 `CACHED`。如果默认应用失败，`RESET_PENDING` 必须保留。

### 12.4 三种重置范围

“当前图片”“从指定起点到末尾”“当前数据集全部图片”是操作范围，不是状态。范围解析完成后，所有目标图片都按同一状态转换处理，并先规范化、去重，再提交失效事务。

### 12.5 实现边界

- 纯状态转换由 `viewport_state_machine.py` 负责，不依赖 Qt。
- Canvas、QScrollArea、缩放控件和滚动条换算由 `ViewportController` facade 负责。
- `LabelingWidget.load_file()` 遵循“解析计划 → 完整应用 → 成功提交”的顺序。
- 视口状态不写入图片、shape、标注 JSON 或用户配置。
- 正常切图为均摊 O(1) 状态查询；批量重置为 O(N) 内存操作，不遍历 shape 或读取图片文件。
