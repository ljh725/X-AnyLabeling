...
│  │  │    ├─ switch_to_person(gid) ──► fill_mode.activate(gid)       │  │  │
...
KeypointToolWindow.__init__(fill_mode, label_widget)
    ...
    │    └─ _connect_signals() ──► 连接 fill_mode 信号（不连接 canvas.new_shape）
    ▼
...
keypoint_tool_window.show() / raise_()
    │
    ▼ (如果有未完成的 person)
content_widget.switch_to_person(min(incomplete_gids))
...
KeypointDockContent.switch_to_prev_person() / switch_to_next_person()
    ...
    │  计算循环索引
    ▼
KeypointDockContent.switch_to_person(gid)
...
| `KeypointDockContent` | `widgets/keypoint_tool_window.py` | 窗口内容组件：人员列表、关键点状态、进度条、自动激活开关 |
