"""Self-contained unit tests for the category layout migration module.

Uses lightweight mock objects for ``PoseLabelItem`` and host constants
so the tests run anywhere with only PyQt6 installed — no host project
import required.  Mirrors the behavioural contract documented in
``README.md``.
...
from migration.pose_category_layout import (
    _body_part_group,
    _head_lateral_rank,
    _is_lateral_prefix,
    layout_category,
...
# -----------------------------------------------------------------------
# Host-constant stand-ins (mirrors X-AnyLabeling pose_constants.py)
# -----------------------------------------------------------------------
