...
def write_reports(
    ...
    """Write one report TXT file to each JSON directory."""
    directories = sorted(
...
def main() -> int:
    ...
    report_paths = write_reports(file_results, load_errors, args.report_name)
...
功能说明：
    ...
    支持多线程并行处理，检测完后会在每个 JSON 文件夹里生成一份 TXT 报告，
    把有问题的文件和具体错误都列出来。

运行命令样例：

  # 基础用法：检测指定目录下的所有 JSON 文件
  python scripts/validate_pose_labels.py ./annotations

  # 检测单个 JSON 文件
  python scripts/validate_pose_labels.py ./annotations/0001.json

  # 使用 8 个线程加速
  python scripts/validate_pose_labels.py ./annotations -w 8

  # 指定报告文件名
  python scripts/validate_pose_labels.py ./annotations --report-name my_report.txt
