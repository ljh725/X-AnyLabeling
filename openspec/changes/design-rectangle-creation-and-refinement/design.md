...
2026-09-10 更新：工具入口与创建收尾以 `unify-rectangle-keyboard-fitting-workflow` 为准。主画面工作台勾选已移除；四极值成功后回到普通编辑，旧单边精修与新增键盘拟合均需显式进入。本文的精修拖动／步进规则只描述独立的旧单边精修，不定义新键盘拟合的按键所有权或 Esc 行为。
...
提交后停留在普通编辑的“新框已选中”状态，不自动进入修正、激活边或改变视图。用户需通过工具菜单或快捷键显式进入单边精修／键盘快速拟合，也可以按“继续新建”进入同标签的下一次四极值输入。“继续新建”复用可配置动作；不能用背景点击隐式触发，以免检查时误画。
