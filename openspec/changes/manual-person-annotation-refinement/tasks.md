...
- [x] 9.1 增加直接边拖动的图像边界、失焦/鼠标抓取丢失回滚、无左键活动拖动和热路径回归测试
- [x] 9.2 直接边拖动遵守图像边界、最小宽高和反翻转约束
- [x] 9.3 统一直接边拖动的提交/取消边界；Esc、失焦、窗口停用和鼠标抓取丢失时回滚
- [x] 9.4 移除 mouse move 无条件同步重绘，并将顶点优先与矩形边候选合并为一次遍历
- [x] 9.5 确保 `Shape.edge_editing` 在绘制异常时也能复位
- [x] 9.6 运行矩形边相关测试、全量回归、py_compile、Black 和目标 Flake8

## 10. 矩形边编辑第二轮重构

- [x] 10.1 增加矩形边状态转换、稳定预览状态、Canvas 选择所有权和无模型渲染状态测试
- [x] 10.2 新增矩形边交互控制器与显式 phase，迁移 mouse pending/dragging 和 keyboard edge 状态
- [x] 10.3 新增稳定预览状态对象与显式 mode，集中 target/drag_locked/window 状态清理
